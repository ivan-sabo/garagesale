...
// Sale represents one item of a transaction where some amount of a product was sold
type Sale struct {
...
// NewSale is what we require from clients for recording new transations
type NewSale struct {
	Quantity int `json:"quantity"`
	Paid     int `json:"paid"`
}
